...
class Recording(val name: String) {
    ...
    val startDate: String = SimpleDateFormat(Option.Save.DATE_FORMAT, Locale.getDefault()).format(Date())
    ...
    fun stopAndSave(context: Context) {
        ...
        val durationStr = String.format(Option.Save.DURATION_FORMAT, hours, minutes, seconds)

        val totalDist = SessionManager.calculateDistance(_points)
        val finalDistanceStr = if (totalDist >= Option.Save.METERS_IN_KILOMETER) {
            String.format(Locale.getDefault(), Option.Save.KM_FORMAT, totalDist / Option.Save.METERS_IN_KILOMETER)
        } else {
            String.format(Locale.getDefault(), Option.Save.M_FORMAT, totalDist)
        }
